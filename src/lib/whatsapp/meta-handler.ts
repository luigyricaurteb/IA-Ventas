...
import { isAudioTranscriptionEnabled, transcribeMetaAudio, buildConfirmationMessage } from "../audio/transcribe";
...
export async function handleMetaMessage(
  ...
  try {
    ...
    if (msg.type === "audio" || (msg.type as string) === "voice") {
      ...
      await sendText(db, phone, "🎤 Escuché tu mensaje de voz, un momento...");

      const transcription = await transcribeMetaAudio(mediaId ?? "", metaToken);
      ...
      // Guardar transcripción en estado para confirmación
      ...
      const confirmMsg = buildConfirmationMessage(transcription);
