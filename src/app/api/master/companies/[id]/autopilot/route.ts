...
export async function GET(req: NextRequest, { params }: Ctx) {
  ...
  const cfg = db.prepare("SELECT autopilot_enabled FROM company_config WHERE id=1").get() as { autopilot_enabled: number } | null;
  return NextResponse.json({ autopilot_enabled: (cfg?.autopilot_enabled ?? 0) === 1 });
}
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  const body = await req.json() as { autopilot_enabled: boolean };
  const db = getCompanyDb(company.slug);
  db.prepare("UPDATE company_config SET autopilot_enabled=? WHERE id=1").run(body.autopilot_enabled ? 1 : 0);
